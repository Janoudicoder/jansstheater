...
?>
<div id="block-<?=$blockId;?>" class="blocken tekst-galerij mb-[13rem] relative container mx-auto galerij  h-[700px] <?= ($catGallery == '1') ? 'tekst-galerij-yallow' : ''; ?> sm:my-28">
    <div class="<?= ($catGallery == '1') ? 'tekstgalerij-background' : ''; ?>">
        <div class="flex flex-wrap lg:flex-nowrap gap-8 h-full">
        <div class="w-full lg:w-1/2 h-full overflow-y-auto flex items-center p-[8px] lg:p-[112px] justify-center">
                <?php
                if ($textGallery) {
            ...
            <div id='galerij-container' class="w-full lg:w-1/2 grid-cols-2 gap-4 !hidden lg:!grid" style="align-content: center;">
                <?php 
                    getImg(get_url(), get_id(), $blockId, get_taal(), 'block', $size['width'], $size['height'], '1440', 4); 
                ?>
            </div>

            <div id='galerij-container' class="galerij-slider w-full !block lg:!hidden" style="align-content: center;">
                <?php 
                    getImg(get_url(), get_id(), $blockId, get_taal(), 'block', $size['width'], $size['height'], '1440', 4); 
                ?>
            </div>
