...
// ===========================
	...
	function saveToDatabase(editableObj,catTaal,id) {
		...
		$.ajax({
			url: "/cms/editinplace/saveedit_categorie.php",
			type: "POST",
...
<div class="fancybox-wrap" style="width:100%; height:100%; display:block;">
    <div class="box-container" style="height: 100%;">
        <div class="box box-full">
            <h3><i class="icon fas fa-language"></i>Vertaal categorie: <?=ucfirst($cat);?></h3>
            <?php
            $sqlCategorie = $mysqli->query("SELECT * FROM sitework_categorie WHERE categorie = '" . $cat . "'") or die($mysqli->error . __LINE__);
            $rowsCategorie = $sqlCategorie->num_rows;

            while ($rowCategorie = $sqlCategorie->fetch_assoc()) {
                $ignoredColumns = ['id', 'categorie', 'datum_aangemaakt']; // Array of columns to exclude
                        ...
                        ?>
                            <div class="form-group">
                                ...
                                <input class="col left inputveld invoer" contenteditable="true"
                                    ...
                                    <?php echo ($columnValue != "") ? 'value="'.$columnValue . '"' : 'placeholder="Voer categorie vertaling in"'; ?>
                                    >
                            </div>
                        <?php
                        endif;
