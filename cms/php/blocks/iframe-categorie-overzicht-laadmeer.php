...
if($taal != 'nl') {
    ...
    $titelCatLaadMeer = getBlockTranslation('titel', $taal, $rowBlockData['id'], '', $cms_id);
    $tekstCatLaadMeer = getBlockTranslation('tekst', $taal, $rowBlockData['id'], $rowBlockData['id'], $cms_id);
    $categorieCatLaadMeer = getBlockTranslation('categorie', $taal, $rowBlockData['id'], $rowBlockData['id'], $cms_id);

} else {
    $blocksData = $mysqli->query("SELECT * FROM sitework_blocks WHERE id = '".$block_id."'") or die($mysqli->error.__LINE__);
    $rowBlockData = $blocksData->fetch_assoc();

    $titelCatLaadMeer = $rowBlockData['titel'];
    $tekstCatLaadMeer = $rowBlockData['tekst'];
    $categorieCatLaadMeer = $rowBlockData['categorie'];
}
        ...
        <form id="contentForm" action="<?php echo $PHP_SELF ?>?blockid=<?=$block_id;?>&taal=<?=$taal;?>&cmsid=<?=$cms_id;?>" method="post" enctype="multipart/form-data">
            <div class="content-container no-border-top mt-0 mb-30 pb-30 float-left">
                <div class="form-group">
                    ...
                    <input type="text" name="titel" class="inputveld invoer" placeholder="" value="<?php echo $titelCatLaadMeer; ?>" />
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="tekst">Selecteer categorie</label>
                    <select name="categorie" class="inputveld invoer dropdown " placeholder="Selecteer een categorie">
                        <option value="<?php echo $categorieCatLaadMeer; ?>"><?php echo $categorieCatLaadMeer; ?></option>
                        <option value="">----------------------------</option>
                        <?php
