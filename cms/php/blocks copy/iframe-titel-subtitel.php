...
if($taal != 'nl') {
    ...
    $subtitel = getBlockTranslation('tekst', $taal, $rowBlockData['id'], '', $cms_id);
    $titel = getBlockTranslation('titel', $taal, $rowBlockData['id'], $rowBlockData['id'], $cms_id);

} else {
    $blocksData = $mysqli->query("SELECT * FROM sitework_blocks WHERE id = '".$block_id."'") or die($mysqli->error.__LINE__);
    $rowBlockData = $blocksData->fetch_assoc();

    $subtitel = $rowBlockData['tekst'];
    $titel = $rowBlockData['titel'];
}
...
<form action="<?php echo $PHP_SELF ?>?blockid=<?=$block_id;?>&taal=<?=$taal;?>&cmsid=<?=$cms_id;?>" method="post" enctype="multipart/form-data">
    <div class="content-container no-border-top mt-0 mb-30 pb-30 float-left">
        <div class="form-group">
            <label for="tekst">Subtitel</label>
            <input type="text" name="tekst" class="inputveld invoer" placeholder="" value="<?php echo $subtitel; ?>" />
        </div>
        <div class="form-group">
            <label for="titel">Titel</label>
            <input type="text" name="titel" class="inputveld invoer" placeholder="" value="<?php echo $titel; ?>" />
        </div>
        <div class="form-group">
