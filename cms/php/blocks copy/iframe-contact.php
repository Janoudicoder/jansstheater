...
if($taal != 'nl') {
    ...
    $titelContactForm = getBlockTranslation('titel', $taal, $rowBlockData['id'], '', $cms_id);
    $tekstContactForm = getBlockTranslation('tekst', $taal, $rowBlockData['id'], $rowBlockData['id'], $cms_id);
    $idContactForm = getBlockTranslation('categorie', $taal, $rowBlockData['id'], $rowBlockData['id'], $cms_id);
} else {
    $blocksData = $mysqli->query("SELECT * FROM sitework_blocks WHERE id = '".$block_id."'") or die($mysqli->error.__LINE__);
    $rowBlockData = $blocksData->fetch_assoc();

    $titelContactForm = $rowBlockData['titel'];
    $tekstContactForm = $rowBlockData['tekst'];
    $idContactForm = $rowBlockData['categorie'];
}
        ...
        <form id="contentForm" action="<?php echo $PHP_SELF ?>?blockid=<?=$block_id;?>&taal=<?=$taal;?>&cmsid=<?=$cms_id;?>" method="post" enctype="multipart/form-data">
            <div class="content-container no-border-top mt-0 mb-30 pb-30 float-left">
                <div class="form-group">
                    ...
                    <input type="text" name="titel" class="inputveld invoer" placeholder="" value="<?php echo $titelContactForm; ?>" />
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="tekst">Selecteer formulier</label>
                    <select name="categorie" class="inputveld invoer dropdown " placeholder="Selecteer een categorie">
                        <?php if($idContactForm){
                            //contact pagina ophalen
                            $sqlSelectedForm = $mysqli -> prepare("SELECT naam FROM sitework_formulieren WHERE id = ?") or die ($mysqli->error.__LINE__);
                            $sqlSelectedForm -> bind_param('i',$idContactForm);
                            $sqlSelectedForm -> execute();
                            $sqlSelectedForm->store_result();
                            $sqlSelectedForm->bind_result($naamFormSelected);
                            $sqlSelectedForm -> fetch();

                            echo '<option value="'.$idContactForm.'">'.$naamFormSelected.'</option>';
                        }else{
                        ...
                        <option value="">----------------------------</option>
                        <?php
                        $sqlCategories = $mysqli->prepare("SELECT id, naam FROM sitework_formulieren ORDER BY naam ASC") or die($mysqli->error . __LINE__);
                        $sqlCategories->execute();
                        $sqlCategories->store_result();
                        $sqlCategories->bind_result($idForm, $naamForm,);
                        while ($sqlCategories->fetch()) {
                                echo '<option value="'.$idForm.'">'.$naamForm.'</option>';
                        }
                        ?>
                    </select>
                </div>
