...
if($taal != 'nl') {
    ...
    $tekstVolBreed = getBlockTranslation('tekst', $taal, $rowBlockData['id'], '', $cms_id);
} else {
    $blocksData = $mysqli->query("SELECT * FROM sitework_blocks WHERE id = '".$block_id."'") or die($mysqli->error.__LINE__);
    $rowBlockData = $blocksData->fetch_assoc();

    $tekstVolBreed = $rowBlockData['tekst'];
}
...
if($_POST['opslaan'] == 1 && $_POST['blockTaal'] == 'true') {
    ...
    $tekstVolBreed = getBlockTranslation('tekst', $_POST['taal'], $rowBlockData['id'], '', $_POST['cmsid']);
}
