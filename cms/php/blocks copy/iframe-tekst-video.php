...
if($taal != 'nl') {
    ...
    $videoTekst = getBlockTranslation('tekst', $taal, $rowBlockData['id'], '', $cms_id);
    $videoCode = getBlockTranslation('titel', $taal, $rowBlockData['id'], $rowBlockData['id'], $cms_id);
    $videoSoort = getBlockTranslation('categorie', $taal, $rowBlockData['id'], $rowBlockData['id'], $cms_id);

} else {
    $blocksData = $mysqli->query("SELECT * FROM sitework_blocks WHERE id = '".$block_id."'") or die($mysqli->error.__LINE__);
    $rowBlockData = $blocksData->fetch_assoc();

    $videoTekst = $rowBlockData['tekst'];
    $videoCode = $rowBlockData['titel'];
    $videoSoort = $rowBlockData['categorie'];
}
    ...
    <body>
        <form id="contentForm" action="<?php echo $PHP_SELF ?>?blockid=<?=$block_id;?>&taal=<?=$taal;?>&cmsid=<?=$cms_id;?>" method="post" enctype="multipart/form-data">
            <div class="content-container no-border-top mt-0 mb-30 pb-30 float-left">
                ...
                <div class="form-group">
                    <label for="titel">Video code</label>
                    <input type="text" name="titel" class="inputveld invoer" placeholder="" value="<?php echo $videoCode; ?>" />
                </div>
                <div class="form-group">
                    <label for="categorie">Soort video</label>
                    <div class="inputveld invoer radio">
                        <input type="radio" name="categorie" id="video-label-yt" value="youtube" <?php if ($videoSoort == "youtube") {echo "checked"; } ?> />
                        <label for="video-label-yt">YouTube video</label>
                        <input type="radio" name="categorie" id="video-label-vi" value="vimeo" <?php if ($videoSoort == "vimeo") {echo "checked"; } ?> />
                        <label for="video-label-vi">Vimeo video</label>
                    </div>
                </div>
                <div class="form-group">
