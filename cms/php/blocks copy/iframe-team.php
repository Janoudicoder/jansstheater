...
if($taal != 'nl') {
    ...
    $titelTeamOverzicht = getBlockTranslation('titel', $taal, $rowBlockData['id'], '', $cms_id);
    $tekstTeamOverzicht = getBlockTranslation('tekst', $taal, $rowBlockData['id'], $rowBlockData['id'], $cms_id);

} else {
    $blocksData = $mysqli->query("SELECT * FROM sitework_blocks WHERE id = '".$block_id."'") or die($mysqli->error.__LINE__);
    $rowBlockData = $blocksData->fetch_assoc();

    $titelTeamOverzicht = $rowBlockData['titel'];
    $tekstTeamOverzicht = $rowBlockData['tekst'];
}
        ...
        <form id="contentForm" action="<?php echo $PHP_SELF ?>?blockid=<?=$block_id;?>&taal=<?=$taal;?>&cmsid=<?=$cms_id;?>" method="post" enctype="multipart/form-data">
            <div class="content-container no-border-top mt-0 mb-30 pb-30 float-left">
                <div class="form-group">
                    ...
                    <input type="text" name="titel" class="inputveld invoer" placeholder="" value="<?php echo $titelTeamOverzicht; ?>" />
                </div>
                <div class="form-group">
